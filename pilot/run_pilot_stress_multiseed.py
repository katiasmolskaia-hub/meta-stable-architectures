"""Run stress test across multiple seeds for sigma levels."""
...
def main() -> None:
    ...
    per_seed_path = out_dir / "pilot_stress_multiseed_per_seed.csv"
    with per_seed_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["sigma", "seed", "model", "success", "failure", "avg_time"])
        writer.writeheader()
        writer.writerows(per_seed_rows)

    summary_path = out_dir / "pilot_stress_multiseed_summary.csv"
    ...
    print("Multi-seed stress test complete.")
    for row in summary_rows:
        print(
            f"sigma={row['sigma']} {row['model']}: success={row['success_mean']}?{row['success_std']} "
