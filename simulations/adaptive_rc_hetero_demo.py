...
def run_demo(params: HeteroDemoParams, seed: int, adaptive: bool = True) -> dict[str, np.ndarray]:
...
def main() -> None:
    ...
    adaptive = run_demo(params, seed=11, adaptive=True)
    fixed = run_demo(replace(params, a1=0.0, a2=0.0, a3=0.0, b1=0.0, b2=0.0), seed=11, adaptive=False)

    def group_rec_time(arr: np.ndarray, t: np.ndarray, threshold: float = 0.2) -> float:
    ...
    def agent_stats(arr: np.ndarray, t: np.ndarray) -> tuple[float, float, float]:
        rt = recovery_times_per_agent(arr, t, params.stress_time + params.stress_duration)
        ok = rt[~np.isnan(rt)]
        if ok.size == 0:
            return float("nan"), float("nan"), float("nan")
        return float(np.mean(ok)), float(np.std(ok)), float(np.percentile(ok, 95))

    a_mean, a_std, a_p95 = agent_stats(adaptive["S"], adaptive["t"])
    f_mean, f_std, f_p95 = agent_stats(fixed["S"], fixed["t"])

    print(f"Adaptive group recovery time: {group_rec_time(adaptive['S'], adaptive['t']):.3f}")
    print(f"Fixed group recovery time: {group_rec_time(fixed['S'], fixed['t']):.3f}")
    print(f"Adaptive agent recovery std: {a_std:.3f}, p95: {a_p95:.3f}")
    print(f"Fixed agent recovery std: {f_std:.3f}, p95: {f_p95:.3f}")
    ...
    try:
        ...
        fig.tight_layout()
        out_plot = out_dir / "adaptive_rc_hetero_demo_plot.png"
