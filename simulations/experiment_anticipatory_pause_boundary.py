...
def full_model_network(topology: str, seed: int, with_pause: bool, with_attention: bool, with_recognition: bool) -> NetworkParams:
    net = NetworkParams(
        n_agents=180,
        t_end=45.0,
        dt=0.025,
    ...
    if with_pause:
        net.forewarning_enabled = True
        net.forewarning_gain = 1.00
        net.forewarning_decay = 0.22
        net.forewarning_trigger = 0.30
        net.forewarning_phase_floor = 0.12
        net.forewarning_phase_ceiling = 0.58
        net.forewarning_s_ceiling = 0.72
        net.pause_peer_suppress = 0.70
        net.pause_instructor_boost = 0.65
        net.pause_anchor_boost = 0.45
        net.pause_slow_gain = 0.30
        net.pause_relax_gain = 0.36
        if with_attention:
            net.pause_attention_mode = True
            net.pause_attention_focus_gain = 0.80
            net.pause_attention_template_gain = 1.30
            net.pause_attention_phi_gain = 0.50
        if with_recognition:
            net.instructor_recognition_enabled = True
            net.recognition_rate = 0.34
            net.recognition_decay = 0.05
            net.recognition_similarity_gain = 8.0
            net.recognition_threshold = 0.46
            net.recognition_attention_boost = 0.55
...
def run_metrics(out: dict[str, np.ndarray], threshold: float = 0.2) -> dict[str, float]:
    ...
    return {
        ...
        "mean_forewarning": float(np.mean(out["mean_forewarning"][start_idx:])),
        "mean_pause": float(np.mean(out["mean_pause"][start_idx:])),
        "mean_recognition": float(np.mean(out["mean_recognition"][start_idx:])),
    }
...
def main() -> None:
    ...
    for sigma in sigmas:
        ...
        for topology in topologies:
            for variant, variant_flags in variants.items():
                with_pause, with_attention, with_recognition = variant_flags
                for seed in seeds:
                    ...
                    rows.append(row)
                    print(
                        f"sigma={sigma:.1f} {topology} {variant} seed={seed}: success={row['success']:.0f}, "
                        f"rt={row['recovery_time']}, tail={row['tail_span']}, err={row['peak_mean_error']:.3f}, "
                        f"turb={row['peak_mean_turbulence']:.3f}, pause={row['mean_pause']:.3f}, recog={row['mean_recognition']:.3f}"
                    )
    ...
    for sigma in sigmas:
        for topology in topologies:
            for variant in variants:
                sub = [row for row in rows if row["sigma_noise"] == sigma and row["topology"] == topology and row["variant"] == variant]
    ...
    csv_path = out_dir / "anticipatory_pause_boundary.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_path = out_dir / "anticipatory_pause_boundary_summary.csv"
    ...
    for topology in topologies:
        for variant, marker in [("full_model", "o"), ("full_model_with_pause", "x"), ("full_model_with_pause_attention", "^"), ("full_model_with_pause_attention_recognition", "s")]:
            sub = [row for row in summary_rows if row["topology"] == topology and row["variant"] == variant]
            label = f"{topology} {variant}"
            axes[0].plot(x, [row["success_rate"] for row in sub], marker=marker, label=label)
            axes[1].plot(x, [row["mean_peak_error"] for row in sub], marker=marker, label=label)
            axes[2].plot(x, [row["mean_peak_turbulence"] for row in sub], marker=marker, label=label)
            axes[3].plot(x, [row["mean_recognition"] if "recognition" in variant else row["mean_pause"] for row in sub], marker=marker, label=label)

    axes[0].set_ylabel("success")
    axes[0].set_title("Boundary recovery with and without anticipatory pause")
    axes[1].set_ylabel("peak error")
    axes[1].set_title("Peak field error")
    axes[2].set_ylabel("peak turbulence")
    axes[2].set_title("Peak turbulence")
    axes[3].set_ylabel("pause / recognition")
    axes[3].set_title("Mean pause or recognition level after stress")
    ...
    fig.tight_layout()
    plot_path = out_dir / "anticipatory_pause_boundary_plot.png"
