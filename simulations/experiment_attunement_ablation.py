...
def main() -> None:
    ...
    variants = {
        "without_attunement": {
            ...
            "initial_attunement": 0.0,
        },
        "with_attunement": {
            "attunement_enabled": True,
            "attunement_gain": 0.08,
            "attunement_decay": 0.010,
            "initial_attunement": np.concatenate([np.full(12, 0.02), np.full(12, 0.06), np.full(12, 0.10)]),
        },
    }
    ...
    for variant, extra in variants.items():
        kg_memory = 0.0
        phi_init = 0.0
        agent_attunement = extra["initial_attunement"]
        for idx, lesson in enumerate(lessons, 1):
            ...
            row = {
                ...
                "initial_group_memory": kg_memory,
                "initial_attunement": float(np.mean(agent_attunement)) if isinstance(agent_attunement, np.ndarray) else float(agent_attunement),
                **metrics,
            }
            rows.append(row)
            ...
            print(
                ...
                f"std={row['agent_std']:.3f}, tail={row['tail_span']:.3f}, "
                f"peak_error={row['peak_mean_error']:.3f}, final_att={row['final_attunement']:.3f}"
            )

    csv_path = out_dir / "attunement_ablation.csv"
    ...
    for variant in variants:
        ...
        axes[3].plot(x, [row["final_attunement"] for row in sub], marker="o", label=variant)
    ...
    axes[3].set_title("Group attunement")
    ...
    fig.tight_layout()
    plot_path = out_dir / "attunement_ablation_plot.png"
