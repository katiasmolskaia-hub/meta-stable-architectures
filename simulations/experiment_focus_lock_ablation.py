...
def lesson_metrics(out: dict[str, np.ndarray], stress_start: float, stress_duration: float, threshold: float = 0.2) -> dict[str, float]:
    ...
    return {
        ...
        "focus_lock_peak": float(np.max(out["mean_focus_lock"])),
...
def main() -> None:
    ...
    for variant, extra in variants.items():
        ...
        for idx, lesson in enumerate(lessons, 1):
            ...
            print(
                ...
                f"focus={row['focus_lock_peak']:.3f}"
            )

    csv_path = out_dir / "focus_lock_ablation.csv"
    ...
    fig.tight_layout()
    plot_path = out_dir / "focus_lock_ablation_plot.png"
