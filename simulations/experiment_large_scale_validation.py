...
def main() -> None:
    ...
    for n_agents in size_variants:
        alpha = make_grouped_values(n_agents, (0.78, 0.86, 0.95))
        sigma_noise = make_grouped_values(n_agents, (0.30, 0.22, 0.14))
        h_crit = make_grouped_values(n_agents, (0.62, 0.69, 0.77))
        initial_attunement = make_grouped_values(n_agents, (0.02, 0.06, 0.10))
        ...
        base_common = dict(
            ...
            spatial_radius=0.18 if n_agents >= 1000 else (0.24 if n_agents >= 300 else 0.30),
        ...
        for variant, extra in model_variants.items():
            variant_rows = []
            kg_memory = 0.0
            phi_init = 0.0
            agent_attunement = initial_attunement.copy()

            for lesson_idx, lesson in enumerate(lesson_cycle, 1):
                ...
                print(
                    f"N={n_agents} {variant} lesson_{lesson_idx}_{lesson['lesson_type']}: "
                    f"rt={row['recovery_time']:.3f}, tail={row['tail_span']:.3f}, "
            ...
            summary_rows.append(
                {
                    "n_agents": n_agents,
                    "variant": variant,
                    "mean_recovery_time": float(np.mean([r["recovery_time"] for r in variant_rows])),
                    "mean_tail_span": float(np.mean([r["tail_span"] for r in variant_rows])),
                    ...
                    "mean_final_attunement": float(np.mean([r["final_attunement"] for r in variant_rows])),
                    "mean_final_demper_load": float(np.mean([r["final_demper_load"] for r in variant_rows])),
                }
            )

    csv_path = out_dir / "large_scale_validation.csv"
    with csv_path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(fh, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)

    summary_path = out_dir / "large_scale_validation_summary.csv"
    ...
    x = np.arange(len(size_variants))
    labels = [str(n) for n in size_variants]
    for variant in model_variants:
        ...
        axes[3].plot(x, [row["mean_final_attunement"] for row in sub], marker="o", label=variant)

    axes[0].set_ylabel("recovery")
    axes[0].set_title("Mean recovery by network size")
    axes[1].set_ylabel("tail")
    axes[1].set_title("Mean tail by network size")
    axes[2].set_ylabel("field")
    axes[2].set_title("Mean error and turbulence by network size")
    axes[3].set_ylabel("attunement")
    axes[3].set_title("Final attunement by network size")
    axes[3].set_xlabel("n_agents")
    ...
    fig.tight_layout()
    plot_path = out_dir / "large_scale_validation_plot.png"
