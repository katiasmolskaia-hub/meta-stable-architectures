...
def episode_metrics(out: dict[str, np.ndarray], start: float, duration: float, threshold: float = 0.2) -> dict[str, float]:
    ...
    idx = min(len(t) - 1, int(np.searchsorted(t, stress_end + 2.0, side="left")))
    return {
        ...
        "frac_t2": float(np.mean(s[idx] > threshold)),
    }
...
def make_params(seed: int = 11) -> tuple[MasterParams, NetworkParams]:
    rng = np.random.default_rng(seed)
    p = MasterParams()
    p.sigma_noise = np.clip(0.22 * (1.0 + 0.60 * rng.normal(size=12)), 0.05, 1.0)
    p.h_crit = np.clip(0.70 * (1.0 + 0.35 * rng.normal(size=12)), 0.35, 1.20)
    p.alpha = np.clip(0.90 * (1.0 + 0.30 * rng.normal(size=12)), 0.50, 1.40)
...
def main() -> None:
    ...
    p, base_net = make_params(seed=11)
    ...
    no_kg_out = simulate_network(p, base_net)
    kg_out = simulate_network(p, kg_net)

    summaries = [summarize(no_kg_out, "network_no_kg"), summarize(kg_out, "network_with_kg")]
    for row in summaries:
        print(
            f"{row['label']}: final_kg={row['final_group_memory']:.3f}, mean_phi={row['mean_phi_gain']:.3f}, "
            f"ep3_rt={row['ep3_rt']:.3f}, ep3_std={row['ep3_agent_std']:.3f}, ep3_p90={row['ep3_p90']:.3f}, "
            f"ep3_tail={row['ep3_tail_span']:.3f}, t+2={row['ep3_tplus2']:.3f}"
        )

    csv_path = out_dir / "network_kg_hetero_tiny_summary.csv"
    ...
    try:
        ...
        fig.tight_layout()
        plot_path = out_dir / "network_kg_hetero_tiny_plot.png"
