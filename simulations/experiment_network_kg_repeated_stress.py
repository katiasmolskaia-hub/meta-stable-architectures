...
def summarize(out: dict[str, np.ndarray], label: str, windows: list[tuple[float, float]]) -> dict[str, float | str]:
    ...
    return {
        ...
        "final_agent_p90": float(rows[-1]["agent_p90"]),
        ...
        "episode1_rt": float(rows[0]["recovery_time"]),
        "episode2_rt": float(rows[1]["recovery_time"]),
        "episode3_rt": float(rows[2]["recovery_time"]),
        "episode4_rt": float(rows[3]["recovery_time"]),
        "episode5_rt": float(rows[4]["recovery_time"]),
    }
...
def main() -> None:
    ...
    for row in summaries:
        print(
            ...
            f"ep1={row['episode1_rt']:.3f}, ep2={row['episode2_rt']:.3f}, ep3={row['episode3_rt']:.3f}, "
            f"ep4={row['episode4_rt']:.3f}, ep5={row['episode5_rt']:.3f}, "
            f"final_kg={row['final_group_memory']:.3f}, mean_phi={row['mean_phi_gain']:.3f}"
        )

    csv_path = out_dir / "network_kg_repeated_stress_summary.csv"
    ...
    try:
        ...
        fig.tight_layout()
        plot_path = out_dir / "network_kg_repeated_stress_plot.png"
