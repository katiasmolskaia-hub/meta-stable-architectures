...
def _energy_proxy(y: np.ndarray, h: np.ndarray, s: np.ndarray) -> np.ndarray:
    # Proxy energy model for network agents.
    # y^2 = fast fluctuation cost, H = accumulated stress, S = isolation load.
    e0 = 0.2
    alpha_y = 0.6
    alpha_h = 0.4
    alpha_s = 0.3
    return e0 + alpha_y * (y**2) + alpha_h * h + alpha_s * s
...
def main() -> None:
    ...
    out_csv = out_dir / "qrc_per_agent_energy.csv"
    out_plot = out_dir / "qrc_per_agent_energy_plot.png"
    out_summary = out_dir / "qrc_per_agent_energy_summary.txt"

    p = _configure_master(sigma_noise=0.22)
    ...
    # Plot: sorted tail + hist
    try:
        import matplotlib.pyplot as plt

        order = np.argsort(e_mean)
        e_sorted = e_mean[order]
        e_eff_sorted = e_eff_mean[order]

        fig, axes = plt.subplots(1, 2, figsize=(10, 4.5))

        axes[0].plot(e_sorted, label="E_mean (sorted)")
        axes[0].plot(e_eff_sorted, label="E_eff_mean (sorted)")
        ...
        axes[1].hist(e_mean, bins=18, alpha=0.6, label="E_mean")
        axes[1].hist(e_eff_mean, bins=18, alpha=0.6, label="E_eff_mean")
