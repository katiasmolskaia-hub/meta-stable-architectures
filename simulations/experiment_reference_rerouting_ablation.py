...
def lesson_metrics(out: dict[str, np.ndarray], stress_start: float, stress_duration: float, threshold: float = 0.2) -> dict[str, float]:
    ...
    return {
        ...
        "peak_focus_lock": float(np.max(out["mean_focus_lock"])),
...
def main() -> None:
    ...
    base_common = dict(
        ...
        focus_lock_trigger=0.22,
        focus_lock_strength=0.95,
        focus_lock_anchor_boost=1.45,
    ...
    variants = {
        ...
        "with_reroute": {
            ...
            "reroute_strength": 1.0,
            "reroute_instructor_weight": 1.35,
            "reroute_anchor_weight": 1.10,
        },
    }
    ...
    for variant, extra in variants.items():
        ...
        for idx, lesson in enumerate(lessons, 1):
            ...
            print(
                f"{variant} {lesson['name']}: rt={row['recovery_time']:.3f}, "
                f"tail={row['tail_span']:.3f}, peak_error={row['peak_mean_error']:.3f}, "
                f"peak_turb={row['peak_mean_turbulence']:.3f}, cascade={row['peak_cascade_fraction']:.3f}"
            )

    csv_path = out_dir / "reference_rerouting_ablation.csv"
    ...
    for variant in variants:
        ...
        axes[1].plot(x, [row["tail_span"] for row in sub], marker="o", label=variant)
        ...
        axes[4].plot(x, [row["peak_cascade_fraction"] for row in sub], marker="o", label=variant)
    ...
    axes[2].set_title("Field error")
    axes[3].set_ylabel("peak turbulence")
    axes[3].set_title("Field turbulence")
    axes[4].set_ylabel("peak cascade")
    axes[4].set_title("Cascade fraction")
    axes[4].set_xlabel("lesson")
    ...
    fig.tight_layout()
    plot_path = out_dir / "reference_rerouting_ablation_plot.png"
